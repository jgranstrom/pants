...
Scala dependency inference now understands Scala 3 `given` and `export` definitions, `derives` clauses, and `given` imports, so cross-file references to them are inferred without an explicit dependency.

Scala dependency inference now resolves references to Scala 3 top-level definitions used from another file in the same package.
